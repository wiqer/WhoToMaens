...
UltraRAG内置了一系列的模型调优方法和评价指标，为了验证这些方法的效果，我们使用垂直领域的数据，在 BugAgaric 上完成了一系列的验证，并在最后给出了相应的分析指标，这些指标验证了UltraRAG 的可行性。

## 评测说明

我们在实验的过程中使用到的模型列表如下所示，BugAgaric 除了支持以下模型以外，其他模型还在陆续完善中。
...
| **BugAgaric-DDR**           | 使用微调前的 embedding 模型，微调前的 LLM 完成实验 |
| **BugAgaric-KBAlign**       | 使用 KBAlign 方法微调模型后完成实验                |
| **VanillaRAG-finetune**    | 使用微调后的 embedding 模型和 微调后的LLM 完成实验 |
| **BugAgaric-Adaptive-Note** | 使用微调前的 embedding+LLM完成实验                 |
...
### **BugAgaric-Adaptive-Note的 case 分析**
